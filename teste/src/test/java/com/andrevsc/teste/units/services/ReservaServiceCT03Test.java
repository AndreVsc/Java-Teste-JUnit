package com.andrevsc.teste.units.services;

import com.andrevsc.teste.exceptions.PagamentoRecusadoException;
import com.andrevsc.teste.models.enums.StatusReserva;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

import java.util.Optional;

import static org.assertj.core.api.Assertions.assertThatThrownBy;
import static org.mockito.ArgumentMatchers.any;
import static org.mockito.ArgumentMatchers.anyString;
import static org.mockito.Mockito.verify;
import static org.mockito.Mockito.when;
import static org.mockito.ArgumentMatchers.argThat;

@DisplayName("CT03 – pagamento recusado no cartão à vista")
class ReservaServiceCT03Test extends ReservaServiceCTBase {

    @Test
    void CT03_pagamentoRecusadoCartaoVista_deveCancelarReserva() {
        when(carroRepository.findById(CARRO_ID)).thenReturn(Optional.of(carroDisponivel()));
        when(detranApiRepository.isCnhValida(anyString())).thenReturn(true);
        when(pagamentoApiRepository.processarPagamento(any())).thenReturn(false);

        assertThatThrownBy(() -> service.reservarCarro(requestValido()))
            .isInstanceOf(PagamentoRecusadoException.class);

        verify(reservaRepository).save(argThat(r -> r.getStatus() == StatusReserva.RESERVA_CANCELADA));
    }
}
